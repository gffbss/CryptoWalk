...
$(document).ready(function(){	
 ...
 //    	url: 'https://btc-e.com/api/2/ltc_usd/ticker',
	...
	// Function responsible for getting the correct click action and applying calling the correspnding function
	$('button').click(function(){
		
		var requestType = $(this).attr('class'),
			gitCoinType = $(this).attr('data-dev'),
			newsCoinType = $(this).attr('data-coin-name');

		if (requestType == "news") {
			dispModal(newsTitle, newsInfo, newsImg, newsEtc, newsBody);
			getNews('https://ajax.googleapis.com/ajax/services/search/news?v=1.0&q=' + newsCoinType +'');
			removeInfo();
			console.log('news');

		} else if (requestType == "repos") {
			dispModal(devTitle, devInfo, devImg, devEtc, devBody);
			queryGit('https://api.github.com/search/repositories?q='+ gitCoinType +'&sort=created');
			removeInfo();
			console.log('repos');	
		}
	...
	// bitcoin.removeInfo();
	// var bitcoinTitle = bitcoin.getTitle();
